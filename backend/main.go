...
func main() {
	...
	mux.Handle("/uploads/", http.StripPrefix("/uploads/", http.FileServer(http.Dir("./uploads")))) // ローカル開発用に残す
...
// ★★★ NEW: Go標準ライブラリを使ったカスタムファイルアップロード関数 ★★★
...
func createPost(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if len(post.ImageURLs) > 0 {
		for _, dataURL := range post.ImageURLs {
			...
			err = uploadFileToSupabase("post-images", filename, decoded, mimeType)
			if err != nil {
	...
	if imageColumn != "" {
		...
		if len(imageURLs) > 0 {
			...
			for _, url := range imageURLs {
				fileNames = append(fileNames, filepath.Base(url))
			}
			err := deleteFileFromSupabase("post-images", fileNames)
			if err != nil {
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var post Post
		rows.Scan(&post.ID, &post.Username, &post.Content, pq.Array(&post.ImageURLs), &post.Label, &post.CreatedAt, &post.GoodCount, &post.BadCount)
...
func createReplyToPost(w http.ResponseWriter, r *http.Request, postID int) {
	...
	db.QueryRow(query, postID, reply.Username, reply.Content).Scan(&reply.ID, &reply.CreatedAt)
	reply.PostID = postID
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusCreated)
...
func createReplyToReply(w http.ResponseWriter, r *http.Request, parentReplyID int) {
	...
	json.NewDecoder(r.Body).Decode(&reply)
	var postID int
	db.QueryRow("SELECT post_id FROM replies WHERE id = $1", parentReplyID).Scan(&postID)
	query := `INSERT INTO replies (post_id, parent_reply_id, username, content) VALUES ($1, $2, $3, $4) RETURNING id, created_at`
	db.QueryRow(query, postID, parentReplyID, reply.Username, reply.Content).Scan(&reply.ID, &reply.CreatedAt)
	reply.PostID = postID
...
func getRepliesForPost(w http.ResponseWriter, r *http.Request, postID int) {
	...
	for rows.Next() {
		...
		rows.Scan(&reply.ID, &reply.PostID, &parentReplyID, &reply.Username, &reply.Content, &reply.CreatedAt, &reply.GoodCount, &reply.BadCount, &parentUsername)
...
func createPostReaction(w http.ResponseWriter, r *http.Request, postID int) {
	...
	db.QueryRow(query, postID, reaction.ReactionType).Scan(&reaction.ID, &reaction.CreatedAt)
...
func createReplyReaction(w http.ResponseWriter, r *http.Request, replyID int) {
	...
	db.QueryRow(query, replyID, reaction.ReactionType).Scan(&reaction.ID, &reaction.CreatedAt)
