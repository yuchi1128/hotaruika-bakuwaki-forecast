...
export default function HourlyForecast({
  ...
  return (
    <div className="relative">
      ...
      {/* スクロールコンテナ */}
      <div
        className="overflow-x-auto rounded-lg border border-white/10 has-custom-scrollbar touch-scroll"
      ...
      >
        <div className="flex">
          {weather.map((w, i) => {
            ...
            const isCurrentHour =
              isTodayPage && now >= itemDate && now < new Date(itemDate.getTime() + 60 * 60 * 1000);
      ...
      {/* カスタムスクロールバー（親が relative なのでここで絶対配置） */}
      {isMobile && <CustomScrollbar containerRef={hourlyForecastRef} />}
