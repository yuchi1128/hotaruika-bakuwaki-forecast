...
export const getWeatherFromCode = (code: number): string => {
  ...
  if (code === 21 || code === 24 || code === 25 || code === 61 || code === 80)
    return "雨";
...
export const getLastUpdateTime = (): Date => {
  ...
  const lastUpdateHourToday = [...updateHours]
    .reverse()
    .find((hour) => currentHour >= hour);
...
export const formatDate = (date: Date) => {
  ...
  const nextDayStr = nextDay.toLocaleDateString("ja-JP", {
    month: "numeric",
    day: "numeric",
    weekday: "short",
  });
...
export const getOffSeasonMessage = (date: Date) => {
  ...
  if (month === 0) {
    return "現在はホタルイカの身投げの時期ではありません。今年の2月から予測を再開します。";
  }
...
export const predictionLevels: PredictionLevel[] = [
  {
    level: 0,
    name: "湧きなし",
    description: "身投げは期待できません",
    color: "text-gray-300",
    bgColor: "bg-gray-500/20 border border-gray-400/20 backdrop-blur-sm",
  },
  {
    level: 1,
    name: "プチ湧き",
    description: "運が良ければ見られるかも",
    color: "text-blue-300",
    bgColor: "bg-blue-500/[.14] border border-blue-400/20 backdrop-blur-sm",
  },
  {
    level: 2,
    name: "チョイ湧き",
    description: "少し期待できるかも",
    color: "text-cyan-300",
    bgColor: "bg-cyan-500/[.14] border border-cyan-400/20 backdrop-blur-sm",
  },
  {
    level: 3,
    name: "湧き",
    description: "それなりに期待できそうです",
    color: "text-green-300",
    bgColor: "bg-green-500/[.14] border border-green-400/20 backdrop-blur-sm",
  },
  {
    level: 4,
    name: "大湧き",
    description: "良い身投げが期待できます！！",
    color: "text-yellow-300",
    bgColor: "bg-yellow-500/[.14] border border-yellow-400/20 backdrop-blur-sm",
  },
  {
    level: 5,
    name: "爆湧き",
    description: "今季トップクラスの身投げが期待できます！！！",
    color: "text-pink-300",
    bgColor: "bg-pink-500/[.14] border border-pink-400/20 backdrop-blur-sm",
  },
];
...
// レベル判定ロジック（共通）
export const getPredictionLevel = (
...
export const getBakuwakiLevelInfo = (predicted_amount: number, date: Date) => {
  ...
  const bakuwakiIndex = Math.min(
    Math.round((predicted_amount / WAKI_THRESHOLDS[0]) * 100),
    200,
  ); // 200%を上限とする
  ...
  const levelInfo =
    predictionLevels.find((p) => p.level === level) || predictionLevels[0];
